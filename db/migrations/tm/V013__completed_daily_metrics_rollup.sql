CREATE TABLE IF NOT EXISTS analytics.snapshot_completed_daily_metrics_facts (
...
BEGIN
  ...
  LOOP
    ...
    v_daily_partition_name := format('snapshot_completed_daily_metrics_facts_p_%s', v_snapshot.snapshot_id);
    ...
    EXECUTE format(
      'CREATE TABLE IF NOT EXISTS analytics.%I (LIKE analytics.snapshot_completed_daily_metrics_facts INCLUDING DEFAULTS INCLUDING CONSTRAINTS)',
    ...
    INTO v_has_daily_rows
    USING v_snapshot.snapshot_id;

    IF NOT v_has_daily_rows THEN
      CALL analytics.populate_snapshot_completed_daily_metrics_rollup_table(
        v_snapshot.snapshot_id,
        v_source_partition,
        v_daily_partition
    ...
    IF NOT EXISTS (
      SELECT 1
      FROM pg_inherits
      WHERE inhparent = 'analytics.snapshot_completed_daily_metrics_facts'::REGCLASS
        AND inhrelid = v_daily_partition
    ) THEN
      EXECUTE format(
        'ALTER TABLE analytics.snapshot_completed_daily_metrics_facts ATTACH PARTITION analytics.%I FOR VALUES IN (%s)',
        v_daily_partition_name,
        v_snapshot.snapshot_id
      );
    END IF;
