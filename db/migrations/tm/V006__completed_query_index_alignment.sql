...
DECLARE
  v_partition RECORD;
  v_snapshot_suffix TEXT;
  v_parent_index_name CONSTANT TEXT := 'ix_snapshot_task_daily_facts_snapshot_date_role_date';
  v_child_index_name TEXT;
BEGIN
  EXECUTE format(
    'CREATE INDEX IF NOT EXISTS %I ON ONLY analytics.snapshot_task_daily_facts(snapshot_id, date_role, reference_date)',
    v_parent_index_name
  );
  ...
  FOR v_partition IN
    ...
    WHERE parent_ns.nspname = 'analytics'
      AND parent.relname = 'snapshot_task_daily_facts'
      AND child_ns.nspname = 'analytics'
    ORDER BY child.relname
  LOOP
    v_snapshot_suffix := substring(v_partition.partition_name from 'snapshot_task_daily_facts_p_(.+)$');
    IF v_snapshot_suffix IS NULL THEN
      RAISE EXCEPTION 'Unexpected task-daily-facts partition name: %', v_partition.partition_name;
    END IF;

    v_child_index_name := format('ix_stdf_p_%s_date_role_date', v_snapshot_suffix);

    EXECUTE format(
      'CREATE INDEX IF NOT EXISTS %I ON analytics.%I(snapshot_id, date_role, reference_date)',
      v_child_index_name,
    ...
    BEGIN
      EXECUTE format(
        'ALTER INDEX analytics.%I ATTACH PARTITION analytics.%I',
        v_parent_index_name,
        v_child_index_name
      );
    EXCEPTION
...
DROP INDEX IF EXISTS analytics.ix_snapshot_task_daily_facts_snapshot_date_role_status_date;
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format(
      'DROP INDEX IF EXISTS analytics.%I',
      format('ix_stdf_p_%s_date_role_status_date', v_snapshot_suffix)
    );
  END LOOP;
END;
$$;
...
BEGIN
  ...
  BEGIN
    ...
    EXECUTE format(
      'CREATE INDEX %I ON analytics.%I(snapshot_id, date_role, reference_date)',
      format('ix_stdf_p_%s_date_role_date', v_snapshot_id),
      v_task_daily_partition_name
    );
    EXECUTE format(
    ...
    EXECUTE format(
      ...
      format('ux_soddf_p_%s_key', v_snapshot_id),
      v_open_due_daily_partition_name
    );
    EXECUTE format(
      'CREATE INDEX %I ON analytics.%I(jurisdiction_label, role_category_label, region, location, task_name, work_type)',
      format('ix_soddf_p_%s_slicers', v_snapshot_id),
      v_open_due_daily_partition_name
    );
    ...
    EXECUTE format(
      ...
      format('ux_stedf_p_%s_key', v_snapshot_id),
      v_task_event_daily_partition_name
    );
