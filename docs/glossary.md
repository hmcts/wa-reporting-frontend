...
- Processing time: Days between task creation and completion.
- Priority bucket: A derived priority label (urgent/high/medium/low) computed from major priority and due date.
- Role category: Role grouping attached to a task (for example, caseworker role category).
- Service: Jurisdiction or service label associated with a task (for example, case type jurisdiction).
- Task audit: A view of completed tasks for a specific case, including assignment history count.
- Task name: Human-readable task name or type (for example, "Review application").
