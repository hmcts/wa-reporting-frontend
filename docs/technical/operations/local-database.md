...
| `yarn db:local:seed` | Seed TM/CRD/LRD reference data, generate source task rows, run snapshot refresh, and publish a local analytics snapshot. |
| `yarn db:local:refresh` | Run only `CALL analytics.run_snapshot_refresh_batch()` for diagnostics. |
...
- The seed path generates upstream `cft_task_db.reportable_task` source rows and lets `analytics.run_snapshot_refresh_batch()` populate snapshot tables, rollups, filters, and indexes.
