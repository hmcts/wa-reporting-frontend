...
const applyCompletedScenario = (row, random, isWithinSla, seedContext) => {
  ...
  return {
    ...
    dueOffset,
    completedOffset,
    lastUpdatedOffset: completedOffset,
...
SELECT
  ...
  CASE WHEN rows.last_updated_offset IS NULL THEN NULL ELSE CURRENT_DATE + rows.last_updated_offset::integer END,
  CASE
