...
import { snapshotOutstandingDueStatusDailyFactsRepository } from '../../../../../main/modules/analytics/shared/repositories';

jest.mock('../../../../../main/modules/analytics/shared/repositories', () => ({
  snapshotOutstandingDueStatusDailyFactsRepository: { fetchTasksDueByDateRows: jest.fn() },
}));
...
describe('tasksDueByDateChartService', () => {
  ...
  test('maps rows into due-by-date points', async () => {
    const filters = {};
    (snapshotOutstandingDueStatusDailyFactsRepository.fetchTasksDueByDateRows as jest.Mock).mockResolvedValue([
    ...
    expect(snapshotOutstandingDueStatusDailyFactsRepository.fetchTasksDueByDateRows).toHaveBeenCalledWith(
      snapshotId,
      filters
    );
  ...
  test('propagates repository errors', async () => {
    ...
    (snapshotOutstandingDueStatusDailyFactsRepository.fetchTasksDueByDateRows as jest.Mock).mockRejectedValue(error);
