...
jest.mock('../../../../../main/modules/analytics/shared/repositories', () => ({
  snapshotOpenDueDailyFactsRepository: { fetchOpenTasksSummaryRows: jest.fn() },
}));
...
describe('openTasksSummaryStatsService', () => {
  ...
  test('returns null when no summary rows are available', async () => {
    ...
    expect(snapshotOpenDueDailyFactsRepository.fetchOpenTasksSummaryRows).toHaveBeenCalledWith(snapshotId, filters);
  ...
  test('propagates repository errors', async () => {
    ...
    (snapshotOpenDueDailyFactsRepository.fetchOpenTasksSummaryRows as jest.Mock).mockRejectedValue(error);
