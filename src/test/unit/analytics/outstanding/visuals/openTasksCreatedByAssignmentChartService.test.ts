...
import { snapshotOutstandingCreatedAssignmentDailyFactsRepository } from '../../../../../main/modules/analytics/shared/repositories';

jest.mock('../../../../../main/modules/analytics/shared/repositories', () => ({
  snapshotOutstandingCreatedAssignmentDailyFactsRepository: { fetchOpenTasksCreatedByAssignmentRows: jest.fn() },
}));
...
describe('openTasksCreatedByAssignmentChartService', () => {
  ...
  test('builds series points and percentages by assignment state', async () => {
    const filters = {};
    (
      snapshotOutstandingCreatedAssignmentDailyFactsRepository.fetchOpenTasksCreatedByAssignmentRows as jest.Mock
    ).mockResolvedValue([
    ...
    expect(
      snapshotOutstandingCreatedAssignmentDailyFactsRepository.fetchOpenTasksCreatedByAssignmentRows
    ).toHaveBeenCalledWith(snapshotId, filters);
  ...
  test('returns zero percentages when open count is zero', async () => {
    ...
    (
      snapshotOutstandingCreatedAssignmentDailyFactsRepository.fetchOpenTasksCreatedByAssignmentRows as jest.Mock
    ).mockResolvedValue([{ date_key: '2024-01-03', assignment_state: 'Assigned', total: 0 }]);
  ...
  test('defaults missing totals to zero', async () => {
    ...
    (
      snapshotOutstandingCreatedAssignmentDailyFactsRepository.fetchOpenTasksCreatedByAssignmentRows as jest.Mock
    ).mockResolvedValue([{ date_key: '2024-01-04', assignment_state: 'Assigned', total: null }]);
