...
describe('taskFactsRepository', () => {
  ...
  test('builds completed summary query for open-ended ranges', async () => {
    ...
    expect(fromQuery.sql).toContain('valid_from_snapshot_id <=');
    ...
    expect(toQuery.sql).toContain('valid_from_snapshot_id <=');
  ...
  test('fetchOverviewFilterOptionsRows executes a single normalized filter-options query', async () => {
    ...
    expect(query.sql).toContain('facts.valid_from_snapshot_id <=');
  ...
  test('applies due/open filters and numeric priority rank in due-priority query', async () => {
    ...
    expect(query.sql).toContain('priority = 5000 AND reference_date < (');
    expect(query.sql).toContain('priority = 5000 AND reference_date = (');
    expect(query.sql).toContain('SELECT batches.as_of_date');
  ...
  test('builds completed by name/location/region queries and range filters', async () => {
    ...
    expect(byLocationQuery.sql).toContain('valid_from_snapshot_id <=');
    ...
    expect(byRegionQuery.sql).toContain('valid_from_snapshot_id <=');
