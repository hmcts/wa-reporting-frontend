import { TasksDuePriorityRow, snapshotOpenDueDailyFactsRepository } from '../../shared/repositories';
...
class TasksDueByPriorityChartService {
  async fetchTasksDueByPriority(snapshotId: number, filters: AnalyticsFilters): Promise<PrioritySeriesPoint[]> {
    const rows: TasksDuePriorityRow[] = await snapshotOpenDueDailyFactsRepository.fetchTasksDuePriorityRows(
      snapshotId,
      filters
    );
    return rows.map(row => ({
